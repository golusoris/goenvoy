...
This is goenvoy's **foundational contract**. The equivalent of golusoris's `PLAN.md §2`, re-cut for a pure-stdlib multi-module client-lib collection. [AGENTS.md](../AGENTS.md) and [CLAUDE.md](../CLAUDE.md) cite this file as the authoritative reference. Any deviation requires a PR comment with justification.

Structure mirrors golusoris §2 so reviewers can move between the two repos without re-orienting.
...
**ADRs** live in [`docs/adr/`](../docs/adr/), Nygard format, one decision per file. Retroactively backfilled records capture every convention that's currently implicit:

| ADR | Title | Status |
|---|---|---|
| [0001](../docs/adr/0001-pure-stdlib.md) | Pure stdlib — no external dependencies | Accepted |
| [0002](../docs/adr/0002-per-module-semver.md) | Per-module independent semver via `<path>/vX.Y.Z` tags | Accepted |
| [0003](../docs/adr/0003-new-constructor-shape.md) | `New(baseURL, apiKey) → *Client` constructor shape | Accepted |
| [0004](../docs/adr/0004-functional-options.md) | Functional-options configuration (`Option` + `With*`) | Accepted |
| [0005](../docs/adr/0005-api-error-type.md) | Per-module `APIError` struct implementing `error` | Accepted |
...
| **Keep a Changelog 1.1** | Root `CHANGELOG.md` — hand-maintained per-module subsections today; release-please can take over per-module later. |
...
**N/A.** goenvoy ships no binaries, no containers, no manifests. The only release artefact is the tagged module source plus supply-chain attestations (see §2.5).
