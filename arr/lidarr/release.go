...
func (c *Client) CreateReleaseProfile(ctx context.Context, rp *arr.ReleaseProfileResource) (*arr.ReleaseProfileResource, error) {
	var out arr.ReleaseProfileResource
	if err := c.base.Post(ctx, "/api/v1/releaseprofile", rp, &out); err != nil {
		return nil, err
	}
	return &out, nil
}
...
func (c *Client) UpdateReleaseProfile(ctx context.Context, rp *arr.ReleaseProfileResource) (*arr.ReleaseProfileResource, error) {
	var out arr.ReleaseProfileResource
	if err := c.base.Put(ctx, fmt.Sprintf("/api/v1/releaseprofile/%d", rp.ID), rp, &out); err != nil {
		return nil, err
	}
	return &out, nil
}
...
// SearchReleases searches for releases matching the given criteria.
func (c *Client) SearchReleases(ctx context.Context, albumID int) ([]arr.ReleaseResource, error) {
