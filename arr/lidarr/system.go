...
// Shutdown sends a shutdown command to Lidarr.
func (c *Client) Shutdown(ctx context.Context) error {
	return c.base.Post(ctx, "/api/v1/system/shutdown", nil, nil)
}

// Restart sends a restart command to Lidarr.
func (c *Client) Restart(ctx context.Context) error {
	return c.base.Post(ctx, "/api/v1/system/restart", nil, nil)
}

// Ping checks connectivity to the Lidarr instance.
...
// BrowseFileSystem returns directory and file listings for a given path.
func (c *Client) BrowseFileSystem(ctx context.Context, path string, includeFiles bool) (*FileSystemResource, error) {
	...
	endpoint := fmt.Sprintf("/api/v1/filesystem?path=%s&includeFiles=%t", url.QueryEscape(path), includeFiles)
...
func (c *Client) GetFileSystemType(ctx context.Context, path string) (string, error) {
	var out string
	endpoint := "/api/v1/filesystem/type?path=" + url.QueryEscape(path)
	if err := c.base.Get(ctx, endpoint, &out); err != nil {
		return "", err
	}
	return out, nil
...
func (c *Client) GetFileSystemMediaFiles(ctx context.Context, path string) ([]string, error) {
	var out []string
	endpoint := "/api/v1/filesystem/mediafiles?path=" + url.QueryEscape(path)
	if err := c.base.Get(ctx, endpoint, &out); err != nil {
		return nil, err
	}
	return out, nil
