...
func (c *Client) CreateQualityProfile(ctx context.Context, p *arr.QualityProfile) (*arr.QualityProfile, error) {
	var out arr.QualityProfile
	if err := c.base.Post(ctx, "/api/v1/qualityprofile", p, &out); err != nil {
		return nil, err
	}
	return &out, nil
}
...
func (c *Client) UpdateQualityProfile(ctx context.Context, p *arr.QualityProfile) (*arr.QualityProfile, error) {
	var out arr.QualityProfile
	if err := c.base.Put(ctx, fmt.Sprintf("/api/v1/qualityprofile/%d", p.ID), p, &out); err != nil {
		return nil, err
	}
	return &out, nil
}
